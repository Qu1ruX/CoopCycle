...
describe('Course e2e test', () => {
  ...
  it('should create and save Courses', async () => {
    ...
    await promise.all([
      courseUpdatePage.setPriceInput('5'),
      courseUpdatePage.setDistanceInput('5'),
      courseUpdatePage.livreurSelectLastOption(),
    ]);

    expect(await courseUpdatePage.getPriceInput()).to.eq('5', 'Expected price value to be equals to 5');
