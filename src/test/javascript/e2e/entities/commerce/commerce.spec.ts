...
describe('Commerce e2e test', () => {
  ...
  it('should create and save Commerce', async () => {
    ...
    await promise.all([
      commerceUpdatePage.setNameInput('Hz'),
      commerceUpdatePage.setReviewsInput('5'),
      commerceUpdatePage.setAddressInput('address'),
      // commerceUpdatePage.cooperativeSelectLastOption(),
    ]);

    expect(await commerceUpdatePage.getNameInput()).to.eq('Hz', 'Expected Name value to be equals to Hz');
    expect(await commerceUpdatePage.getReviewsInput()).to.eq('5', 'Expected reviews value to be equals to 5');
    expect(await commerceUpdatePage.getAddressInput()).to.eq('address', 'Expected Address value to be equals to address');
