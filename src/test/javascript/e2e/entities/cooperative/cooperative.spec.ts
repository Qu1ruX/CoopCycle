...
describe('Cooperative e2e test', () => {
  ...
  it('should create and save Cooperatives', async () => {
    ...
    await promise.all([cooperativeUpdatePage.setNameInput('Kx'), cooperativeUpdatePage.setAddressInput('address')]);

    expect(await cooperativeUpdatePage.getNameInput()).to.eq('Kx', 'Expected Name value to be equals to Kx');
    expect(await cooperativeUpdatePage.getAddressInput()).to.eq('address', 'Expected Address value to be equals to address');
